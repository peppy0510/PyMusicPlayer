...
'''
#-------------------------------------------------------------------------------
import os, sys, ctypes, platform, packctrl
libraryName =  'bass'
trialPaths = ['', 'packages']
if sys.platform.startswith('win'):
	bass_module, func_type = packctrl.tryLoadDLL(\
		'%s.dll' % (libraryName), trialPaths)
elif sys.platform.startswith('darwin'):
	bass_module, func_type = packctrl.tryLoadDLL(\
		'lib%s.dylib' % (libraryName), trialPaths)
#-------------------------------------------------------------------------------
...
if __name__ == "__main__":
	...
	if not BASS_Init(-1, 44100, 0, 0, 0):
		...
		handle = BASS_StreamCreateFile(False, b'test_pybass.mp3', 0, 0, 0)
		play_handle(handle)
		if not BASS_Free():
