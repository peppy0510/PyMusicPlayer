...
def init_bass_play():
    plugins = ['bassaac.dll', 'bassac3.dll', 'bassape.dll',
...
def get_channel(path):
    # input is filename
    hstream = pybass.BASS_StreamCreateFile(False, path, 0, 0,
                                           pybass.BASS_STREAM_DECODE | pybass.BASS_SAMPLE_MONO | pybass.BASS_UNICODE)
    fs = get_fs(hstream)
    ...
    hstream = pybass.BASS_StreamCreateFile(False,
                                           path, 0, 0, pybass.BASS_STREAM_DECODE | pybass.BASS_UNICODE)
